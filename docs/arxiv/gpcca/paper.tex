...
\subsection{Joint fitting: penalized EY loss by a single L-BFGS-B run over every view}

Because \eqref{eq:encoder} is linear in $B_i$ given the fixed basis $k(X_i, Z_i)$, substituting it into the multiview objective \eqref{eq:ey} for every view at once, plus an RKHS-norm ridge penalty per view, gives
\begin{equation}
\mathcal{L}(B_1, \dots, B_M) = \mathcal{L}_{EY}(Z_1, \dots, Z_M) + \frac{\alpha}{2}\sum_{i=1}^M\sum_{c=1}^k B_i[:,c]^\top K_{mm}^{(i)}\, B_i[:,c],
\label{eq:penalized}
\end{equation}
an ordinary smooth function of every view's coefficients $B_1, \dots, B_M$ jointly, with an exact analytic gradient with respect to each $B_i$ obtained by the chain rule through \eqref{eq:encoder} and the known gradient of $\mathcal{L}_{EY}$ \citep{chapman2024unconstrained}:
\begin{equation}
\nabla_{B_i}\mathcal{L} = k(X_i, Z_i)^\top \nabla_{Z_i}\mathcal{L}_{EY} + \alpha\, K_{mm}^{(i)}\, B_i, \qquad i = 1, \dots, M.
\label{eq:grad}
\end{equation}
...
Every view's coefficients are fit \emph{jointly}: $B_1, \dots, B_M$ are concatenated into a single vector and optimized by one L-BFGS-B run against \eqref{eq:penalized}--\eqref{eq:grad} directly (so no explicit Hessian is needed), rather than cycling through views and re-optimizing each one to convergence with the others held fixed. Since $\mathcal{L}_{EY}$ already couples every view together, solving one view at a time to convergence before moving to the next (a block Gauss--Seidel scheme) repeats work for no benefit and can settle into a worse joint optimum than descending the exact joint gradient directly; a single joint solve also converges in noticeably less wall-clock time in practice, since it does not re-solve each view's sub-problem to full convergence once per sweep. Coefficients are initialized from a cheap random-projection whitening of each view's own basis, and L-BFGS-B runs until the objective's relative change falls below a tolerance $\texttt{tol}$ (passed through as its own \texttt{ftol}) or a maximum number of iterations $\texttt{max\_iter}$ is reached. This is the same L-BFGS-B routine \texttt{scikit-learn}'s own \texttt{GaussianProcessRegressor} uses internally for its marginal-likelihood optimization, just pointed at a different, jointly-coupled objective.
...
We next construct two views of $n=4500$ samples sharing a nonlinear latent factor ($X_2$ driven by the square of the same latent $z$ that drives $X_1$), and fit \texttt{GaussianProcessCCA} with \texttt{n\_inducing} $\in \{20, 50, 100, 200, 400\}$ on $4000$ training samples, reporting held-out canonical correlation on $500$ test samples and wall-clock fit time for each setting (Figure~\ref{fig:scaling}), against exact inference on a subsample of $800$ training points (the largest exact fit we ran within this paper's compute budget, since exact inference is $O(n^3)$ and does not scale to the full $4000$-sample training set within a similar budget). Held-out correlation rises from $0.929$ at \texttt{n\_inducing}$=20$ to $0.934$ by \texttt{n\_inducing}$=100$ and is essentially flat thereafter (up to $0.934$ at \texttt{n\_inducing}$=400$), matching (already at \texttt{n\_inducing}$=20$) or exceeding (from \texttt{n\_inducing}$=50$ on) the $0.929$ held-out correlation of exact inference on the $800$-point subsample. Fit time grows with \texttt{n\_inducing} (from $0.12$s at $20$ inducing points to $4.3$s at $400$), consistent with the $O(nm^2+m^3)$ scaling of Section~\ref{sec:encoder}: even the cheapest sparse setting tried ($m=20$, $0.12$s) already matches exact inference's held-out quality on $800$ points ($2.7$s) at roughly $22\times$ less fit time, while training on $5\times$ as much data.
...
\subsection{Illustrating predictive uncertainty}
\label{sec:uncertainty-fig}

Section~\ref{sec:uncertainty} noted that \texttt{GaussianProcessCCA}'s predictive uncertainty comes for free from the same kernel and inducing points as the fitted mean. To make this concrete we fit \texttt{GaussianProcessCCA} on two views built from a single scalar location $x$: view 1 is $x$ itself, and view 2 is $y = \sin(2.5x)$ plus small noise, so the two views are genuinely correlated and the joint fit has something to learn. Training points are drawn from two clusters, $x \in [-2.5,-0.5]$ and $x \in [0.5, 2.5]$ ($15$ points each), leaving open space beyond both edges. Figure~\ref{fig:uncertainty} plots view 1's fitted encoder $f_1$ against $x$ on a grid spanning well beyond the training range: the posterior mean, a $95\%$ credible band ($\pm 1.96$ posterior standard deviations), four posterior sample paths, and the training points' own projected values. The sample paths are drawn from the same (uncentred) GP prior already used for the posterior standard deviation (Section~\ref{sec:uncertainty}) --- since that covariance depends only on the kernel, the noise level, and the inducing points, never the fitted coefficients, reusing it to draw correlated sample paths is exactly consistent with how the pointwise standard deviation is already computed. The band stays tight and the sample paths stay bunched together wherever training data is nearby, and both flare up sharply beyond the training range ($|x| > 2.5$), where the mean also reverts away from the fitted curve --- the same qualitative behaviour familiar from ordinary one-dimensional GP regression, here obtained for a nonlinear multiview CCA encoder rather than a supervised regression target.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig3_uncertainty.pdf}
\caption{View 1's fitted encoder on the synthetic $x$/$\sin(2.5x)$ task: posterior mean, $95\%$ credible band, four posterior sample paths, and the training points' own projected values. The band and sample paths stay tight near the two training clusters ($x \in [-2.5,-0.5] \cup [0.5,2.5]$) and flare up beyond them, exactly like the familiar one-dimensional GP regression plot.}
