...
class DVCCA(nn.Module):
    ...
    A couple of important variables here, both_encoders and private. Both_encoders is something I added so that we
    could potentially compare the effect of using Private is as described in the paper and adds another encoder for
    private information for each view. For this reason the hidden dimensions passed to the decoders is 3*latent_dims
    as we concanate shared,private_1 and private_2
    """
    ...
    def __init__(self, config: Config = Config):
        ...
        self.encoder_optimizers = [optim.Adam(list(encoder.parameters()), lr=config.learning_rate) for encoder in
                                   self.encoders]
        self.decoder_optimizers = [optim.Adam(list(decoder.parameters()), lr=config.learning_rate) for decoder in
                                   self.decoders]
        if self.private:
            self.private_encoder_optimizers = [optim.Adam(list(encoder.parameters()), lr=config.learning_rate) for
                                               encoder in self.private_encoders]
    ...
    def forward(self, *args):
        ...
        if self.private:
            ...
            z_p = mu_p
            z = tuple([torch.cat([z[i], z_p[i]], dim=-1) for i, _ in enumerate(args)])
        return z
