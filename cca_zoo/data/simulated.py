...
class LinearSimulatedData:
    ...
    def __init__(self, view_features: List[int], latent_dims: int = 1,
                 view_sparsity: Union[List[float], float] = None,
                 correlation: Union[List[float], float] = 0.99,
                 structure: str = "random", positive: Union[bool, List[bool]] = False,
                 random_state: Union[int, np.random.RandomState] = None):

        self.view_features = view_features
        self.latent_dims = latent_dims
        self.random_state = check_random_state(random_state)
        self.correlation = _process_parameter("correlation", correlation, 0.99, latent_dims)
        self.view_sparsity = _process_parameter("view_sparsity", view_sparsity, 0.5, len(view_features))
        self.structure = _process_parameter("structure", structure, "random", len(view_features))
    ...
    def _generate_joint_covariance(self, cov_matrices):
        ...
        for i, j in itertools.combinations(range(len(split_points) - 1), 2):
            ...
            joint_cov[split_points[i]: split_points[i + 1], split_points[j]: split_points[j + 1]] = cross_cov
            joint_cov[split_points[j]: split_points[j + 1], split_points[i]: split_points[i + 1]] = cross_cov.T
    ...
    def _compute_cross_cov(self, cov_matrices, i, j):
        ...
        for _ in range(self.latent_dims):
            outer_product = np.outer(self.true_features[i][:, _], self.true_features[j][:, _])
            cross_cov += cov_matrices[i] @ (self.correlation[_] * outer_product) @ cov_matrices[j]

        return cross_cov

    def _generate_covariance_matrices(self):
        """Generates a list of covariance matrices and true features for each view."""
        cov_matrices = [self._generate_covariance_matrix(f, s) for f, s in zip(self.view_features, self.structure)]
        true_features = [self._generate_true_feature(cov, s, pos) for cov, s, pos in zip(cov_matrices, self.view_sparsity, self.positive)]
    ...
    def _decorrelate_weights(weights, cov):
        ...
        for k in range(1, product.shape[0]):
            weights[:, k:] -= np.outer(weights[:, k - 1], product[k - 1, k:] / product[k - 1, k - 1])
            product = weights.T @ cov @ weights
        return weights
