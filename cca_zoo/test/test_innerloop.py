...
def test_regularized():
    ...
    park_gen = cca_zoo.models.innerloop.ParkhomenkoInnerLoop(
        c=[0.0001, 0.0001], generalized=True
    ).fit(X, Y)
    params = {"c": [2, 2]}
    pmd = cca_zoo.models.innerloop.PMDInnerLoop(c=[2, 2]).fit(X, Y)
    pmd_gen = cca_zoo.models.innerloop.PMDInnerLoop(c=[2, 2], generalized=True).fit(
        X, Y
    )
