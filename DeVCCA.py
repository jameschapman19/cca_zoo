...
class DeVCCA(nn.Module, ABC):
    ...
    def __init__(self, input_size_1: int, input_size_2: int, input_size_c: int, hidden_layer_sizes_1: list = None,
        ...
        if self.private:
            self.decoder_1 = cca_zoo.deep_models.Decoder(hidden_layer_sizes_1, input_size_c + latent_dims * 3,
                                                         input_size_1).double()
            self.decoder_2 = cca_zoo.deep_models.Decoder(hidden_layer_sizes_2, input_size_c + latent_dims * 3,
                                                         input_size_2).double()
        else:
            self.decoder_1 = cca_zoo.deep_models.Decoder(hidden_layer_sizes_1, input_size_c + latent_dims,
                                                         input_size_1).double()
            self.decoder_2 = cca_zoo.deep_models.Decoder(hidden_layer_sizes_2, input_size_c + latent_dims,
                                                         input_size_2).double()
    ...
    def decode_1(self, z_1, x_c):
        # 2*latent_dims
        z = torch.cat([z_1, x_c], dim=-1)
        x = self.decoder_1(z)
        return x

    def decode_2(self, z_2, x_c):
        # 2*latent_dims
        z = torch.cat([z_2, x_c], dim=-1)
        x = self.decoder_2(z)
        return x
    ...
    def forward(self, x_1, x_2, x_c):
        ...
        if self.private:
            ...
            z_p2 = mu_p2
            z_1 = torch.cat([z_1, z_p1], dim=-1)
            z_2 = torch.cat([z_2, z_p2], dim=-1)
        return z_1, z_2
    ...
    def loss(self, x_1, x_2, x_c):
        ...
        recon_1 = self.decode_1(z_1, x_c)
        recon_2 = self.decode_2(z_2, x_c)

        # LOSS

        BCE_1 = F.mse_loss(recon_1, x_1, reduction='mean')
        BCE_2 = F.mse_loss(recon_2, x_2, reduction='mean')

        # KL bit - we have assumed logvar diagonal
        KL_1 = -0.5 * torch.mean(1 + logvar_1 - logvar_1.exp() - mu_1.pow(2))
        if self.both_encoders:
            KL_2 = -0.5 * torch.mean(1 + logvar_2 - logvar_2.exp() - mu_2.pow(2))

        if self.private:
            KL_p1 = -0.5 * torch.mean(1 + logvar_p1 - logvar_p1.exp() - mu_p1.pow(2))
            KL_p2 = -0.5 * torch.mean(1 + logvar_p2 - logvar_p2.exp() - mu_p2.pow(2))
            if self.both_encoders:
