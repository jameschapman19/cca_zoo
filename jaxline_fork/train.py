...
def _initialize_experiment(experiment_class, mode, rng, experiment_kwargs):
    ...
    if "init_rng" in init_args:
        experiment = experiment_class(mode, init_rng=rng, **experiment_kwargs)
    else:
        ...
        if writer is not None:
            ...
            periodic_actions += (
                utils.PeriodicAction(
                    write_scalars,
                    interval_type=(
                        config.logging_interval_type or config.interval_type
                    ),
...
def evaluate(
    ...
    eval_rng = jax.pmap(
        functools.partial(
            utils.specialize_rng_host_device,
            axis_name="i",
            mode=config.random_mode_eval,
        ),
        axis_name="i",
    )(eval_rng, host_id_devices)
    ...
    while True:
        ...
        if (
            ...
            and config.eval_initial_weights
        ...
        if config.best_model_eval_metric and jax.host_id() == 0:
            ...
            if eval_metric_is_better_op(
                ...
                logging.info(
                    "%s: %s %s %s, saving new best checkpoint.",
        ...
        if not experiment.should_run_step(state.global_step, config):
            logging.info(
                "Last checkpoint (iteration %d) evaluated, exiting.", state.global_step
            )
