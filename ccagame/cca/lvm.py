...
def forward_model(U_, V_, Z, X, Y):
    ...
    Y_hat = Z @ V_
    return -jnp.mean(
        jnp.sum((X_hat - X) ** 2, axis=1) + jnp.sum((Y_hat - Y) ** 2, axis=1)
    )
...
def backward_model(U, V, Z, X, Y):
    ...
    Z_hat_Y = Y @ V
    return -jnp.mean(
        jnp.sum((Z_hat_X - Z) ** 2, axis=1) + jnp.sum((Z_hat_Y - Z) ** 2, axis=1)
    )
...
def calc_lvm(X, Y, k, iterations=100, lr=1, random_state=0):
    n = X.shape[0]
