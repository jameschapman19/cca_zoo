...
import random
os.chdir('./results/mnist_results/211026')
# %%
...
random.seed(42)
idx = random.sample(list(range(0, train.shape[0])), 500)
train_1 = train[idx, :392]
train_2 = train[idx, 392:]
...
# %%
plt.figure()
plt.suptitle('MNIST results for batch size: {0}'.format(batch_size))
plt.subplot(1,2,1)
...
plt.legend()
plt.savefig("testing_{0}.png".format(batch_size))