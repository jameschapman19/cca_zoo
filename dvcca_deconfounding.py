...
class DEVCCA(DCCA_base):
    ...
    def __init__(self):
        ...
        if config.private:
            ...
            self.decoders = nn.ModuleList(
                [model(config.latent_dims * 3+config.input_sizes[-1], config.input_sizes[i],
                       norm_output=True, **config.decoder_args[i]) for i, model in enumerate(config.decoder_models)])
        else:
            self.decoders = nn.ModuleList([model(config.latent_dims+config.input_sizes[-1], config.input_sizes[i],
    ...
    def decode(self, z, confounds):
        x = []
        z = torch.cat([z, confounds], dim=-1)
    ...
    def recon(self, *args):
        z = self(*args)
        return [self.decode(z_i, args[-1]) for z_i in z][0]
    ...
    def vcca_loss(self, *args, mu, logvar):
        ...
        recon = self.decode(z, args[-1])
        bces = torch.stack([F.binary_cross_entropy(recon[i], args[i], reduction='sum') / batch_n for i, _ in
    ...
    def vcca_private_loss(self, *args, mu, logvar):
        ...
        recon = self.decode(z_combined, args[-1])
        bces = torch.stack([F.binary_cross_entropy(recon[i], args[i], reduction='sum') / batch_n for i, _ in
                            enumerate(self.decoders)]).sum()
        return kl + kl_p + bces
